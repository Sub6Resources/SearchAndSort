...
namespace SearchAndSort
{
    public class KamikazeTank : EnemyTank
    {
        ...
        public override void Move(KeyboardState state)
        {
            ...
            else if (colliding && kamiCharge) Explode();
            switch ((int) targetDirection)
            {
                case (int) UP:
                    MoveUp(false, false);
            ...
            {
                if (location.X >= tank.location.X - AI_TOLERANCE && location.X <= tank.location.X + AI_TOLERANCE &&
                    location.Y >= tank.location.Y - AI_TOLERANCE &&
                    location.Y <= tank.location.Y + AI_TOLERANCE) Explode();
                if (location.X >= tank.location.X - AI_TOLERANCE && location.X <= tank.location.X + AI_TOLERANCE &&
                    location.Y > tank.location.Y)
                {
                    ...
                    if (!kamiCharge) targetDirection = UP;
                }
                    ...
                    location.Y < tank.location.Y)
                {
                    ...
                    if (!kamiCharge) targetDirection = DOWN;
                }

                //If Y = Y of enemy tank and X > X of enemy tank, go left.
                if (location.Y >= tank.location.Y - AI_TOLERANCE && location.Y <= tank.location.Y + AI_TOLERANCE &&
                    location.X > tank.location.X)
                {
                    ...
                    if (!kamiCharge) targetDirection = LEFT;
                }
                    ...
                    location.X < tank.location.X)
                {
                    ...
                    if (!kamiCharge) targetDirection = RIGHT;
                }
            }
